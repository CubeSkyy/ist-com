...
char *mklbl(long);
char *mkfunc(char *);
void variable(int, int, char *, Node*, int);
void func(int, int, char*, Node*, int), externs();
int checkReturn(Node*);
void add_extrn(char*);
char *mkfunc(char *s);
static int ncicl;
static char *fpar;
...
finit   : '(' { arg_pos = 8;} params ')' blocop { $$ = binNode(FUNC, $5, $3); }
	| '(' ')' blocop        { $$ = binNode(FUNC, $3, nilNode(NIL)); }
	;
...
params	: param					  { $$ = binNode(PARAMS, nilNode(NIL), $1); }
	| params ',' param      { $$ = binNode(PARAMS, $1, $3); }
	;

bloco	: '{' { IDpush(); } decls list end '}'    { $$ = binNode(BLOCK, $5 ? binNode(END, $4, $5) : $4, $3); IDpop(); }
	;

decls	:                       { $$ = nilNode(NIL); }
	| decls param ';'       { $$ = binNode(END, $1, $2); }
	;

param	: tipo ID               { $$ = binNode(PARAM, $1, strNode(ID, $2));
                                  if (IDlevel() == 1){
																		IDnew($1->value.i, $2, arg_pos);
																		if ($1->value.i % 10 == 3) arg_pos+=8;
																		else	arg_pos+=4;
																		fpar[++fpar[0]] = $1->value.i;
																	}else{
																		if ($1->value.i % 10 == 3) IDnew($1->value.i, $2, pos -= 8);
																		else	IDnew($1->value.i, $2, pos -= 4);
																	}
                                }
	;
...
list	: base      { $$ = binNode(END, nilNode(NIL), $1); }
	| list base     { $$ = binNode(END, $1, $2); }
	;

args	: expr		{ $$ = binNode(PARAMS,$1, nilNode(NIL)); }
	| args ',' expr { $$ = binNode(PARAMS, $3, $1); }
	;
...
lv	: ID		{ long pos; int typ = IDfind($1, &pos);
	...
	| ID '[' expr ']' { Node *n;
                            ...
                            if (pos == 0) n = strNode(ID, $1);
														...
														if (typ == 2) $$ = binNode(S_INDEX, n, $3);
														else $$ = binNode(INDEX, n, $3);
			    									if (typ >= 10) typ -= 10;
...
expr	: lv		{ $$ = uniNode(PTR, $1); $$->info = $1->info; }
	| '*' lv        { $$ = uniNode(PTR, uniNode(PTR, $2)); if ($2->info % 5 == 2) $$->info = 2; else if ($2->info / 10 == 1) $$->info = $2->info % 10; else yyerror("can dereference lvalue"); }
	...
	| expr '!'             { $$ = uniNode(FACT, $1); $$->info = 3; intonly($1, 0); }
	...
	| expr '+' expr { $$ = binNode(PLUS, $1, $3); $$->info = nostring($1, $3); }
	| expr '-' expr { $$ = binNode(MINUS, $1, $3); $$->info = nostring($1, $3); }
	| expr '*' expr { $$ = binNode(MUL, $1, $3); $$->info = nostring($1, $3); }
	| expr '/' expr { $$ = binNode(DIV, $1, $3); $$->info = nostring($1, $3); }
	| expr '%' expr { $$ = binNode(MOD, $1, $3); $$->info = intonly($1, 0); intonly($3, 0); }
	| expr '<' expr { $$ = binNode(LT, $1, $3); $$->info = 1; }
	| expr '>' expr { $$ = binNode(GT, $1, $3); $$->info = 1; }
	| expr GE expr  { $$ = binNode(GE, $1, $3); $$->info = 1; }
	| expr LE expr  { $$ = binNode(LE, $1, $3); $$->info = 1; }
	| expr NE expr  { $$ = binNode(NE, $1, $3); $$->info = 1; }
	| expr '=' expr { $$ = binNode(EQ, $1, $3); $$->info = 1; }
	| expr '&' expr { $$ = binNode(AND, $1, $3); $$->info = intonly($1, 0); intonly($3, 0); }
	| expr '|' expr { $$ = binNode(OR, $1, $3); $$->info = intonly($1, 0); intonly($3, 0); }
	| '(' expr ')' { $$ = $2; $$->info = $2->info; }
...
void declare(int pub, int cnst, Node *type, char *name, Node *value)
{
	...
	variable(pub, cnst, name, value, type->value.i);
  int typ;
  if (!value) {
...
void function(int pub, Node *type, char *name, Node *body, int enter)
{
	...
	IDpop();
	if (pub && bloco->attrib == NIL) add_extrn(mkfunc(name));
	...
	if (bloco->attrib != NIL) { /* not a forward declaration */
		...
		func(pub, type->value.i, name,body, enter);
	}
}
